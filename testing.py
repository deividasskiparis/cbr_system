...
def test_1():
    ...
    data_all = np.genfromtxt(fname='dataFromKaggle/train301N.csv', dtype=np.float, skip_header=1, delimiter=',')
...
def test_2():
    ...
    data_all = np.genfromtxt(fname='dataFromKaggle/train79N.csv', dtype=None, skip_header=1, delimiter=',')
    X, y = np.hsplit(data_all, np.array([data_all.shape[1] - 1]))
    y = y.astype(np.float, copy=False)
